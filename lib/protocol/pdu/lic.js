...
function productInformation() {
	var self = {
        ...
        pbCompanyName : new type.BinaryString(new Buffer.from('Microsoft Corporation', 'ucs2'), { readLength : new type.CallableValue(function() {
        ...
        pbProductId : new type.BinaryString(new Buffer.from('A02', 'ucs2'), { readLength : new type.CallableValue(function() {
...
function serverLicenseRequest(opt) {
	var self = {
		...
		serverRandom : new type.BinaryString(new Buffer.from(Array(32 + 1).join('\x00')), { readLength : new type.CallableValue(32) } ),
...
function clientNewLicenseRequest(opt) {
	var self = {
        ...
        clientRandom : new type.BinaryString(new Buffer.from(Array(32 + 1).join('\x00')), { readLength : new type.CallableValue(32) }),
...
function serverPlatformChallenge(opt) {
	var self = {
        ...
        MACData : new type.BinaryString(new Buffer.from(Array(16 + 1).join('\x00')), { readLength : new type.CallableValue(16) })
	};
...
function clientPLatformChallengeResponse(opt) {
	var self = {
        ...
        MACData : new type.BinaryString(new Buffer.from(Array(16 + 1).join('\x00'), 'binary'), { readLength : new type.CallableValue(16) })
	};
