...
function orderCapability(orders, opt) {
	...
	var self = {
		...
		terminalDescriptor : new type.BinaryString(new Buffer.from(Array(16 + 1).join('\x00'), 'binary'), {readLength : new type.CallableValue(16)}),
...
function inputCapability(opt) {
	var self = {
        ...
        imeFileName : new type.BinaryString(new Buffer.from(Array(64 + 1).join('\x00'), 'binary'), {readLength : new type.CallableValue(64)})
	};
...
function capability(cap, opt) {
	var self = {
		...
		capability : cap || new type.Factory(function(s) {
			...
			default:
				let capsType = ""
				for (let type in CapsType){
					if (CapsType[type]==self.capabilitySetType.value){
						capsType = type
					}
				}
				log.debug(`Not implement capability set ${capsType} value:0x00${self.capabilitySetType.value.toString(16)} `);
