...
function demandActivePDU(capabilities, opt) {
	var self = {
		...
		sourceDescriptor: new type.BinaryString(new Buffer.from('node-rdpjs', 'binary'), {
...
function confirmActivePDU(capabilities, shareId, opt) {
	var self = {
		...
		sourceDescriptor: new type.BinaryString(new Buffer.from('rdpy', 'binary'), {
			readLength: new type.CallableValue(function () {
				return self.lengthSourceDescriptor.value
			})
		}),
...
function deactiveAllPDU(opt) {
	var self = {
		...
		sourceDescriptor: new type.BinaryString(new Buffer('rdpy', 'binary'), {
			readLength: new type.CallableValue(function () {
...
function bitmapData(coord, opt) {
	...
	var self = {
		...
		bitmapDataStream: new type.BinaryString(coord.data, {
			readLength: new type.CallableValue(function () {
...
function fastPathUpdatePDU(updateData, opt) {
	var self = {
		...
		compressionFlags: new type.UInt8(null, {
			conditional: function () {
		...
		updateData: updateData || new type.Factory(function (s) {
			...
			switch (self.updateHeader.value & 0xf) {
				...
				default:
					self.updateData = new type.BinaryString(null, options).read(s);
					let fpuType = ""
					for (let type in FastPathUpdateType) {
						if (FastPathUpdateType[type] == (self.updateHeader.value & 0xf)) {
							fpuType = type
						}
					}
					log.debug(`Not implement fast path pdu type ${fpuType} value:${(self.updateHeader.value & 0xf)}`);
			}
		})
	};
