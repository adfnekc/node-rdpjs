...
function masterSecret (secret, random1, random2) {
	var sh1 = saltedHash(new Buffer.from('A'), secret, random1, random2);
	var sh2 = saltedHash(new Buffer.from('BB'), secret, random1, random2);
	var sh3 = saltedHash(new Buffer.from('CCC'), secret, random1, random2);
	
	var ms = new Buffer.alloc(sh1.length + sh2.length + sh3.length);
	sh1.copy(ms);
...
function macData(macSaltKey, data) {
	var salt1 = new Buffer.alloc(40);
	salt1.fill(0x36);
	
	var salt2 = new Buffer.alloc(48);
	salt2.fill(0x5c);
...
function rdpInfos(extendedInfoConditional) {
	var self = {
        ...
        domain : new type.BinaryString(new Buffer.from('\x00', 'ucs2'),{ readLength : new type.CallableValue(function() {
        	return self.cbDomain.value + 2;
        })}),
        userName : new type.BinaryString(new Buffer.from('\x00', 'ucs2'), { readLength : new type.CallableValue(function() {
        	return self.cbUserName.value + 2;
        })}),
        password : new type.BinaryString(new Buffer.from('\x00', 'ucs2'), { readLength : new type.CallableValue(function () {
        	return self.cbPassword.value + 2;
        })}),
        alternateShell : new type.BinaryString(new Buffer.from('\x00', 'ucs2'), { readLength : new type.CallableValue(function() {
        	return self.cbAlternateShell.value + 2;
        })}),
        workingDir : new type.BinaryString(new Buffer.from('\x00', 'ucs2'), { readLength : new type.CallableValue(function() {
...
function rdpExtendedInfos(opt) {
	var self = {
	    ...
	    clientAddress : new type.BinaryString(new Buffer.from('\x00', 'ucs2'),{ readLength : new type.CallableValue(function() {
	    	return self.cbClientAddress;
	    }) }),
	    cbClientDir : new type.UInt16Le(function() {
        	return self.clientDir.size();
        }),
	    clientDir : new type.BinaryString(new Buffer.from('\x00', 'ucs2'), { readLength : new type.CallableValue(function() {
	    	return self.cbClientDir;
	    }) }),
	    clientTimeZone : new type.BinaryString(new Buffer.from(Array(172 + 1).join("\x00"))),
...
function reverse(buffer) {
	var result = new Buffer.alloc(buffer.length);
...
Client.prototype.sendClientNewLicenseRequest = function(licenseRequest) {
	...
	var preMasterSecretEncryptedPadded = new Buffer.alloc(preMasterSecretEncrypted.length + 8);
	...
	request.obj.ClientUserName.obj.blobData.value = new Buffer.from(this.machineName + '\x00');
...
Client.prototype.sendClientChallengeResponse = function(platformChallenge) {
	...
	var sig = new Buffer.alloc(serverChallenge.length + hwid.length);
