...
function clientCoreData(opt) {
	var self = {
		...
		clientName : new type.BinaryString(new Buffer.from('node-rdpjs\x00\x00\x00\x00\x00\x00', 'ucs2'), { readLength : new type.CallableValue(32) }),
		...
		imeFileName : new type.BinaryString(new Buffer.from(Array(64 + 1).join('\x00')), { readLength : new type.CallableValue(64), optional : true }),
		...
		clientDigProductId : new type.BinaryString(new Buffer.from(Array(64 + 1).join('\x00')), { optional : true, readLength : new type.CallableValue(64) }),
...
function writeConferenceCreateRequest (userData) {
    ...
    return new type.Component([
	    ...
	    per.writeOctetStream(new Buffer.from(h221_cs_key), 4), per.writeOctetStream(userDataStream.getValue())
    ]);
}
...
function writeConferenceCreateResponse (userData) {
	 ...
	 return new type.Component([
	    ...
	    per.writeNumberOfSet(1), per.writeChoice(0xc0),
	    per.writeOctetStream(new Buffer.from(h221_sc_key), 4), per.writeOctetStream(userDataStream.getValue())
    ]);
}
