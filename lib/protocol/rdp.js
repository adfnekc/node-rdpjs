 ...
 * Main RDP module
 */
class RdpClient extends events.EventEmitter {
	constructor(config) {
		super();
		config = config || {};
		...
		// credentials
		if (config.domain) {
			this.sec.infos.obj.domain.value = new Buffer.from(config.domain + '\x00', 'ucs2');
		}
		if (config.userName) {
			this.sec.infos.obj.userName.value = new Buffer.from(config.userName + '\x00', 'ucs2');
		}
		if (config.password) {
			this.sec.infos.obj.password.value = new Buffer.from(config.password + '\x00', 'ucs2');
		}
		if (config.workingDir) {
			this.sec.infos.obj.workingDir.value = new Buffer.from(config.workingDir + '\x00', 'ucs2');
		}
		if (config.alternateShell) {
			this.sec.infos.obj.alternateShell.value = new Buffer.from(config.alternateShell + '\x00', 'ucs2');
		}
		...
		this.global.on('connect', function () {
			...
			self.emit('bitmaps', { len: bitmaps.length });
 ...
 */
class RdpServer extends events.EventEmitter {
